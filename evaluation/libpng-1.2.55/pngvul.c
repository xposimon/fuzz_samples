...
char *input_file_name = "default.in";
int fuzz_status = 0;
...
#  endif
...
#ifdef PNG_iCCP_SUPPORTED
   {
      if (fuzz_status){
	      ...
	      size_t len = fread(buffer, 1, 200, input_fp);
	      ...
	      printf("name finish\n");
	      png_charp profile = tmp2;
	      int cnt;
              int compression_type=buffer[idx+1];
	      for (cnt = 0; cnt < 100; cnt++)
	      {
		  profile[cnt] = buffer[cnt+idx+2];
		  if (buffer[cnt+idx+2] == '\x00')
		      break;
	      }
	      png_uint_32 proflen=cnt;
	      
	      
	      
	      if (png_get_iCCP(read_ptr, read_info_ptr, &name, &compression_type,
		              &profile, &proflen))
	      printf("---------------CVE 2015-8540 fuzz----------------\n");
              printf("name: %s\n profile: %s\nlength: %d\ntype:%d", name, profile, proflen, compression_type);
	      
	      png_write_iCCP(write_ptr, name, compression_type,
		      profile, proflen);
      } }
   }
...
main(int argc, char *argv[])
{
   ...
   // Compile cmd: gcc -O2 -g -DNDEBUG -fsanitize=address -DLIBPNG_NO_MMX -DPNG_CONFIGURE_LIBPNG -DPNG_NO_CONSOLE_IO -DPNG_NO_MMX_CODE -DPNG_NO_STDIO -DZLIB_DLL -I/home/xposimon/Desktop/evaluation/libpng-1.2.55 png.c pngvul.c pngwrite.c pngread.c pngwutil.c libpng12.a -lm -lz -o pngtest
   fprintf(STDERR,"fuzz args: ./elf -fz [input_file]\n");
   ...
   if (argc > 1)
   {
      ...
      if (strcmp(argv[1], "-fz") == 0)
      {
         
         fuzz_status = 1;
         input_file_name = argv[2];
         inname = "pngtest.png";
         test_one_file(inname, outname);
         return 0;
      }
